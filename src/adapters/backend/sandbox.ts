...
export function prepareDirectSandbox(opts: {
  ...
  // for deny masks. mode 000 → for a non-root uid the mask itself is
  // unreadable/unlistable (a real deny reads as EPERM, not "empty"). A root uid
  // bypasses DAC (CAP_DAC_OVERRIDE) and can still traverse it, but the source is
  // an EMPTY dir/file, so no real content is ever exposed either way — emptiness,
  // not the permission bits, is the load-bearing guarantee.
