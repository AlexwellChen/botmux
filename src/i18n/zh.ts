...
export const messages: Record<string, string> = {
  ...
  'cmd.reply_mode.usage': '用法：/reply-mode chat｜chat-topic｜topic｜new-topic\nchat=群内平铺回复且复用群会话（连原生话题也并进这个会话）；chat-topic=顶层平铺复用群会话，但群里原生话题各自独立会话（默认）；topic=回复落在话题里但复用同一个群 session（shared 是兼容别名）；new-topic=每次 @ 都开一个独立话题+独立会话。仅 chat-topic 会隔离用户创建的原生话题。',
