...
The main path no longer requires a manual `/introduce`: botmux discovers bots in the current Lark group via the group bot roster by default. **Cross-deployment** recognition instead goes through the federation in the Dashboard "Teams" panel (a trust roster is only established when you invite someone else's deployment into the same team and create cross-deployment groups). A plain `/group` or the Dashboard "New Group" flow creates an ordinary same-deployment group, without cross-deployment team trust.
