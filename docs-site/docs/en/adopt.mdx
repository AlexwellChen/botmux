...
- **Restore after a daemon restart is careful**: on restart botmux first re-validates whether the adopted host CLI is still alive — it only closes the session if the target is confirmed gone; on an inconclusive (transient) probe it keeps the session closed until the next wake-up and re-validates, using debouncing so a single network hiccup doesn't spuriously declare "the CLI disconnected" and kill a live process.
