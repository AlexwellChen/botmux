...
现在主路径不再需要手动 `/introduce`：botmux 会默认通过飞书群机器人列表自动发现当前群里的 bot。**跨部署**互相识别则走 Dashboard「团队」面板的联邦机制（把别人的部署拉进同一团队、跨部署拉群时才建立信任花名册）；普通 `/group`、Dashboard「新建群」建的是本部署内的普通群，不带跨部署团队信任。
