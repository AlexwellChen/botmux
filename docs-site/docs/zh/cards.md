...
- **关闭时给「可恢复」卡片**：附上 CLI 原生 resume 命令（如 `claude --resume <id>`）＋「▶️ 恢复会话」按钮，随时点回来继续。

> **打开终端 = 只读**：卡片主按钮「🖥️ 打开 Web 终端」是只读查看；要**可写操作**点「🔑 获取操作链接」（经私聊发可写链接）。「🔄 重启」「接管配置」等管理按钮在**会话卡**上，不在每轮的流式卡上。
