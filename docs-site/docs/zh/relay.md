...
在你想接力**到**的那个群里发 `/relay`，弹出一张卡片，列出**你作为 owner、同一机器人在别处的活跃会话**（排除当前所在话题；同群里的**其它话题**会话也会列出，可用于同群话题间搬运），选一个拉过来即可。
...
- 目标**落点**若已有该 bot 的活跃会话，需先 `/close` 再接力（按落点锚点判定：chat-scope 群即整群，话题群则是具体那条话题——同群不同话题不冲突）。
